...
finderController.getNatureData = (req, res, next) => {
    let now = new Date();
    let weekAgo = new Date(now.getTime() - 604800)
    let weekAgoStr = `${weekAgo.getFullYear()}-${weekAgo.getMonth()}-${weekAgo.getDate()}`
    console.log(weekAgoStr)
    let displayDate = `${weekAgo.toLocaleString('default', { month: 'short' })} ${weekAgo.getDate()}, ${weekAgo.getFullYear()}`
